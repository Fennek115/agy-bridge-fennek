...
| `distill_corpus`  | Distill a book/PDF into the RAG-lite base (one md/chapter + index) per the `destilar-corpus` method. Two modes via `session_id`: ingest (writes `00-ficha.md`, returns chapter list) / chapter (writes `cap-NN.md` from the ingested session). agy writes to disk; returns only the path. Optional `rules` param appends per-project hardening (e.g. "Resumen = one prose paragraph, no bullets; sentences <30 words") without re-pasting the template — the baked template stays generic. | Pro High → Flash High            |
